...
/**
 * RegionalTcDeploymentEntity - JPA Entity for ap_regional_tc_deployments table
 * 
 * Tracks when regional directors deploy approved plans to their tax centers
 * This is DIFFERENT from ap_regional_deployments which tracks director sending to regions
 */
@Entity
@Table(name = "ap_regional_tc_deployments", uniqueConstraints = {
    @UniqueConstraint(name = "uk_regional_tc_deployment_per_plan", columnNames = {"plan_id", "region_id"})
})
public class RegionalTcDeploymentEntity {
    ...
    // Constructors
    public RegionalTcDeploymentEntity() {
    }

    public RegionalTcDeploymentEntity(UUID planId, String regionId, String deployedBy) {
        this.planId = planId;
