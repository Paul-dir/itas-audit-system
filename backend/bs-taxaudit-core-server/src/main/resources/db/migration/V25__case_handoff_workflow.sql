...
CREATE UNIQUE INDEX uk_case_auditor_active ON case_auditor_assignment(case_id) WHERE status = 'ACTIVE';
...
-- ============================================================================
ALTER TABLE team_members ADD COLUMN IF NOT EXISTS id UUID DEFAULT gen_random_uuid();
ALTER TABLE team_members ADD COLUMN IF NOT EXISTS team_leader_id UUID REFERENCES users(id) ON DELETE CASCADE;
ALTER TABLE team_members ADD COLUMN IF NOT EXISTS auditor_id UUID REFERENCES users(id) ON DELETE CASCADE;
ALTER TABLE team_members ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP;
ALTER TABLE team_members ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP;

-- Compound index for active member queries: (team_leader_id, is_active, left_at)
CREATE INDEX IF NOT EXISTS idx_team_members_team_leader ON team_members(team_leader_id, is_active, left_at);

-- Index for reverse lookups: which teams does an auditor belong to
CREATE INDEX IF NOT EXISTS idx_team_members_auditor ON team_members(auditor_id, team_leader_id);

-- Filtered index for active members only (C3 bug fix: empty team detection)
CREATE INDEX IF NOT EXISTS idx_team_members_active ON team_members(team_leader_id) WHERE is_active = true AND left_at IS NULL;
