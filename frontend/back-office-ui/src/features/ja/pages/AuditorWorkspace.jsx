  ...
  const renderStepContent = () => {
    switch (activeStep) {
      ...
      case 'ENTRY_CONFERENCE':
        return <EntryConferencePanel
          caseId={caseId}
          caseData={caseData}
          conference={workflow.conference}
          onSchedule={(conf) => actions.scheduleConference(caseId, user.id, conf)}
          onRecordMinutes={(mins) => { actions.recordMinutes(caseId, user.id, mins); setActiveStep('INFO_REQUEST'); }}
          onSkip={(reason) => { actions.skipConference(caseId, user.id, reason); setActiveStep('INFO_REQUEST'); }}
        />;
