...
export default function PlanDetailModal({ plan, onClose }) {
  ...
  const activeRegions = (cachedConfig?.regions || REGIONS).filter(r => r && r.active !== false);

  const feedbackCount = Object.keys(displayPlan.regionalFeedback || {}).length;
  const pendingRegions = activeRegions.filter(r => !displayPlan.regionalFeedback?.[r.id] && !displayPlan.regionalFeedback?.[r.code]);
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-gray-50 dark:bg-slate-700 rounded-xl p-4 space-y-3 dark:bg-slate-700">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between"><span className="text-gray-500 dark:text-slate-400">Regions</span><span className="font-medium dark:text-white">{activeRegions.length}</span></div>
                  ...
                  <div className="space-y-1.5">
                    {activeRegions.map(r => (
                      <div key={r.id} className="flex items-center justify-between text-xs">
                        ...
                        {(displayPlan.regionalFeedback?.[r.id] || displayPlan.regionalFeedback?.[r.code])
          ...
          {tab === 'feedback' && (
            <div className="space-y-4">
              ...
              {activeRegions.map(region => {
                const fb = displayPlan.regionalFeedback?.[region.id] || displayPlan.regionalFeedback?.[region.code];
                return (
